...
const storage: StorageEngine = multer.diskStorage({
    destination: function (req, file, cb) {
        let dir = "";
        if (file.fieldname === "agreementFile1" || file.fieldname === "agreementFile2" || file.fieldname === "agreementFile3" || file.fieldname === "agreementFile4") {
    ...
    filename: function (req, file, cb) {
        ...
        if (file.fieldname === "agreementFile1") {
            req.body.agreementFileName1 = `${Date.now()}-${
                file.originalname
            }`;
